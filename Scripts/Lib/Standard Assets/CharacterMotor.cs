...
public class CharacterMotor : MonoBehaviour
{
    ...
    {
		//加了一个判定条件(grounded &&!jumping.jumping)让其连续按跳可以跳跃
		if (!inputJump || !canControl || (grounded &&!jumping.jumping))
        {
            jumping.holdingJumpButton = false;
        ...
        else
        {
			...
			velocity.y = Mathf.Min(velocity.y, movement.maxRiseSpeed);
        }
    ...
    {
        //Debug.Log(string.Format(tr.name+"\n({0},{1},{2})", hit.normal.x,hit.normal.y,hit.normal.z));
