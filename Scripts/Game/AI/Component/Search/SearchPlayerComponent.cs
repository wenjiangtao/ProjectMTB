...
namespace MTB
{
    public class SearchPlayerComponent : ISearchComponent
    {
        private GameObject _host;

        public SearchPlayerComponent(GameObject host)
        {
            this._host = host;
        }
        ...
        public GameObject searchTarget(int radius, int mindis, int groupId, bool checkHeight = true)
        {
			...
			List<GameObject> list = HasActionObjectManager.Instance.getManager(HasActionObjectManagerTypes.PLAYER).listObj();
        ...
        {
            Debug.LogError("SearchPlayerComponent暂时不支持这个功能!");
            return null;
        }
