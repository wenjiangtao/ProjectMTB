...
namespace MTB
{
    public class CaveHorizontalGen : CaveGenBase
    {
        private bool _evenCaveDistribution = false;

        public CaveHorizontalGen(int seed)
            : base(seed)
        {
        }
        ...
        protected override void generateChunk(Vector3 chunkCoord, Chunk chunk)
        {
            ...
            for (int j = 0; j < i; j++)
            {
                ...
                double y;
                int count = AreaCaveSum / 2 - 2;
                if (_evenCaveDistribution)
                    y = this._random.Next(CaveMaxAltitude - CaveMinAltitude) + CaveMinAltitude + 10;
                else
